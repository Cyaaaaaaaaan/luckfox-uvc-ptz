...
echo ""
echo "  Binary : $UVC_BIN"
echo "  Scripts: $UVC_SCRIPTS_DIR/usb_config.sh"
RKLUNCH_SH="$(dirname "$(realpath "$0")")/RkLunch.sh"

echo ""
echo "  ── Deploy to Luckfox Pico Pro (via SSH/SCP) ──"
echo "  # 0. (One-time) Back up factory RkLunch.sh"
echo "  ssh root@\$PICO_IP \"cp /oem/usr/bin/RkLunch.sh /oem/usr/bin/RkLunch.sh.factory 2>/dev/null || true\""
echo ""
echo "  # 1. Copy files to board"
echo "  scp \"$UVC_BIN\" \"$UVC_SCRIPTS_DIR/usb_config.sh\" root@\$PICO_IP:/oem/usr/bin/"
...
echo ""
echo "  # 3. One-time board setup (disables S99usb0config — USB networking daemon"
echo "  #    that interferes with UVC by calling S50usbdevice restart on disconnect)"
echo "  ssh root@\$PICO_IP \"chmod -x /etc/init.d/S99usb0config\""
echo ""
echo "  # 4. Test immediately (without reboot)"
echo "  ssh root@\$PICO_IP \"RkLunch-stop.sh 2>/dev/null; kill -9 \$(ps | grep RkLunch | grep -v grep | awk '{print \$1}') 2>/dev/null; /oem/usr/bin/RkLunch.sh &\""
echo ""
echo "  # 5. Reboot to verify autostart"
